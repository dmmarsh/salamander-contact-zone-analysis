#hatchling proportion in Redbacks as a function of site type
#includes site type as a fixed effect (category) and pass/date as a random effect
#so model is stagebin ~ location + (1|pass/date)

library(rstan)

#process data to add index variable for site type
dat<-rbhatch
summary(rbhatch)
...
int<lower=0,upper=1> y[N];              //stage class 1/0
...
stan_run = stan(data = list(N = nrow(dat),
                            ...
                            N_surveys = 8,  #number of survey passes
...
#proportion of posterior distribution below zero (i.e. reduced hatchlings in contact zone)
mean(post$'b[2]'<0)
mean(post$'b[3]'<0)
mean(post$'b[4]'<0)
